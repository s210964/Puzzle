...
class Board {
  ...
  counterMoves = () => {
    ...
    if (this.moves == 0) counterMoves.innerText = `${this.moves} RUCHÓW`;
    else if (this.moves == 1) counterMoves.innerText = `${this.moves} RUCH`;
    else counterMoves.innerText = `${this.moves} RUCHY`;
  };
  ...
  imgUrl = (imgNumber) => `./images/img${imgNumber}.jpg`;
  ...
  moveBlock = (target, shuffling = 0) => {
    ...
    if (shuffling == false) this.verifyBoard();
  };
    ...
    for (i = 0; i < this.board.length; i++) {
      for (j = 0; j < this.board.length; j++) {
        ...
        if (this.board[i][j] == 0) {
          ...
          let tagImg = `<img alt='img' class='imgBlock' style = 'object-position: ${
            this.blockImg[this.board[i][j]]
          }' src = '${this.imgUrl(imgNumber)}'/>`;
        ...
        if (i != 0) upperBlock = this.board[i - 1][j];

        if (i != 3) lowerBlock = this.board[i + 1][j];

        if (j != 0) leftBlock = this.board[i][j - 1];

        if (j != 3) rightBlock = this.board[i][j + 1];

        if (
          upperBlock == emptyBlock ||
          lowerBlock == emptyBlock ||
          leftBlock == emptyBlock ||
          rightBlock == emptyBlock
        ) {
          block.addEventListener("click", ({ target }) => {
            if (this.shuffled == true) this.moveBlock(target.parentElement);
          });
...
let imgNumber = Math.floor(Math.random() * 5);
imgNumber += 1;
...
document
  .querySelector("#imgHelp")
  .setAttribute("src", `./images/img${imgNumber}.jpg`);